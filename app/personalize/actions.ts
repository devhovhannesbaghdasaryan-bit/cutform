...
export async function generatePersonalizedNightLightAction(
  ...
  if (!user) redirect(`/login?next=/personalize/${encodeURIComponent(rawModelId)}`);
  ...
  if (modelError || !model) return errorState(t('errorModel'));
  ...
  if (
    ...
    selectedBoilerplates.length !== requestedBoilerplateIds.length
  ) {
  ...
  try {
    ...
    const requestPayload = buildPersonalizedNightLightOpenAiPayload({
      ...
      ledColor,
      multiColor,
      comfortableColors: PERSONALIZED_NIGHT_LIGHT.comfortableLedColors.map((color) => ({
        ...color,
      })),
    });
    ...
    if (debited && creditSupabase) {
      try {
        ...
        console.error('[personalized-night-light] credit refund failed', refundError);
      }
    }
    return errorState(
