          ...
          {optionAssets.length ? (
            <section className="min-w-0 space-y-4">
              <h2 className="break-words text-lg font-semibold tracking-tight sm:text-xl">
                Personalized preview options
              </h2>
              <div className="grid min-w-0 gap-5 xl:grid-cols-2">
                {optionAssets.map((option) => (
                  <article
                    key={option.id}
                    className="min-w-0 space-y-4 overflow-hidden rounded-lg border p-3 sm:p-4"
                  >
                    <div>
                      <p className="font-semibold">{getOptionName(option)}</p>
                      <p className="text-sm text-muted-foreground">
                        Option {option.option_index} · {option.status}
                      </p>
                    </div>
                    <AssetPreviewCard
                      title="Generated preview"
                      path={option.preview_image_path}
                      url={option.previewUrl}
                      downloadUrl={option.previewDownloadUrl}
                    />
                    {option.manufacturing_file_path ? (
                      <AssetPreviewCard
                        title="Manufacturing PNG"
                        path={option.manufacturing_file_path}
                        url={option.manufacturingFileUrl}
                        downloadUrl={option.manufacturingFileDownloadUrl}
                      />
                    ) : (
                      <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
                        No manufacturing PNG has been generated by an admin.
                      </div>
                    )}
            ...
            <section className="grid gap-4 sm:grid-cols-2">
              <AssetPreviewCard
                title="Generated preview"
                path={parentPreviewPath}
                url={parentPreviewUrl}
                downloadUrl={parentPreviewDownloadUrl}
              />
              {item.manufacturing_file_path ? (
                <AssetPreviewCard
                  title="Manufacturing PNG"
                  path={item.manufacturing_file_path}
                  url={parentManufacturingFileUrl}
                  downloadUrl={parentManufacturingFileDownloadUrl}
                />
              ) : null}
            </section>
          ) : null}

          {item.svg_content.trim() ? (
            <Snapshot title="Raw manufacturing SVG" value={item.svg_content} />
          ) : null}
          {hasContent(item.manufacturing_metadata) ? (
            <Snapshot title="Manufacturing metadata" value={item.manufacturing_metadata} />
          ) : null}
          {hasContent(item.generation_options) ? (
            <Snapshot title="Generation options" value={item.generation_options} />
          ) : null}
          {validationWarnings.length ? (
            <Snapshot title="Validation warnings" value={validationWarnings} />
          ) : null}
          {artifacts?.length ? (
                ...
                {artifacts.map((artifact) => (
                  <div key={artifact.id} className="rounded-lg border p-4">
                    ...
                    {artifact.content_text ? (
                      <Snapshot title="Content" value={artifact.content_text} />
                    ) : null}
                  </div>
                ))}
              </div>
...
function buildManufacturingPrompt(
  item: GeneratedItemAdminDetail,
  option: AdminGeneratedPreviewOption,
) {
  void item;
  void option;
