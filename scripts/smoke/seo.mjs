...
const requiredFiles = ['app/robots.ts', 'app/sitemap.ts', 'lib/seo.ts', 'lib/supabase/middleware.ts'];
for (const file of requiredFiles) {
  if (!existsSync(file)) throw new Error(`Missing SEO file: ${file}`);
}

const seoSource = readFileSync('lib/seo.ts', 'utf8');
for (const symbol of ['resolveCatalogMetadata', 'getAlternateLanguages', 'createProductStructuredData']) {
...
for (const required of ['APP_LOCALES', 'listCategories', 'listPublishedCatalogItems', 'getCanonicalUrl']) {
...
for (const disallowed of ['/admin', '/cart', '/checkout', '/dashboard', '/orders', '/api']) {
  if (!robotsSource.includes(disallowed)) throw new Error(`Robots file does not disallow ${disallowed}`);
}
...
if (!landingPage.includes('generateMetadata')) throw new Error('Landing page is missing generateMetadata');
...
if (!catalogPage.includes('generateMetadata')) throw new Error('Catalog page is missing generateMetadata');
...
if (!itemPage.includes('generateMetadata')) throw new Error('Item page is missing generateMetadata');
if (!itemPage.includes('application/ld+json')) throw new Error('Item page is missing product structured data');
