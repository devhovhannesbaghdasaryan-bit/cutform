...
import { AlignCenter, AlignLeft, Bold, Check, ImagePlus, Italic, LoaderCircle, WandSparkles, X } from 'lucide-react';
import {
...
export function PersonalizedNightLightForm({
  ...
  const [state, formAction, pending] = useActionState(generatePersonalizedNightLightAction, initialState);
  ...
  const selectedNames = boilerplates.filter((item) => selected.includes(item.id)).map((item) => item.name);
    ...
    >
      ...
      <section className="space-y-3">
        ...
        {boilerplates.length ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="relative aspect-[4/3] overflow-hidden bg-muted">
                    ...
                    <img src={option.imageUrl} alt="" className="h-full w-full object-cover transition group-hover:scale-[1.02]" />
                    <span className={`absolute right-2 top-2 grid h-7 w-7 place-items-center rounded-full border bg-background shadow ${checked ? 'border-primary bg-primary text-primary-foreground' : ''}`}>
          ...
          <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">{copy.noTemplates}</div>
        )}
        {!selectedCount && boilerplates.length > 0 ? <p className="text-sm text-muted-foreground">{copy.selectAtLeastOne}</p> : null}
      </section>
      ...
      <section className="space-y-2">
        ...
        <label className="relative flex min-h-36 cursor-pointer flex-col items-center justify-center gap-2 overflow-hidden rounded-lg border border-dashed bg-muted/30 p-5 text-center text-sm text-muted-foreground transition hover:bg-muted/50">
          {filePreview ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={filePreview} alt="" className="absolute inset-0 h-full w-full object-cover opacity-25" />
          ) : null}
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          {PERSONALIZED_NIGHT_LIGHT.comfortableLedColors.map((color) => (
            <label key={color.value} className="flex cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm has-[:checked]:border-primary has-[:checked]:bg-primary/5">
              <input type="radio" name="ledColor" value={color.value} defaultChecked={color.value === PERSONALIZED_NIGHT_LIGHT.defaultLedColor} />
              <span className="h-4 w-4 rounded-full border shadow-inner" style={{ backgroundColor: color.hex }} />
              {color.label}
            </label>
          ))}
        </div>
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between gap-4">
          <Label htmlFor="customTextEditor">{copy.text} <span className="font-normal text-muted-foreground">({copy.textOptional})</span></Label>
          <span className="text-xs text-muted-foreground">{copy.charactersRemaining.replace('{count}', String(remaining))}</span>
        </div>
        <div className="overflow-hidden rounded-md border bg-background">
          ...
          <div className="relative">
            {!text ? <span className="pointer-events-none absolute left-3 top-3 text-sm text-muted-foreground">{copy.textPlaceholder}</span> : null}
            <div ref={editorRef} id="customTextEditor" contentEditable role="textbox" aria-multiline="true" className="min-h-24 px-3 py-2 text-sm outline-none" onInput={syncEditor} onBlur={syncEditor} suppressContentEditableWarning />
          </div>
        </div>
      </section>

      {state.code === 'error' && state.message ? <div role="alert" className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">{state.message}</div> : null}
      ...
      <div className="sticky bottom-3 rounded-xl border bg-background/95 p-3 shadow-lg backdrop-blur">
        ...
        <Button type="submit" className="w-full" disabled={pending || !selectedCount || !fileName}>
          {pending ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <WandSparkles className="mr-2 h-4 w-4" />}
          {pending ? copy.generatingTitle : copy.generate.replace('{count}', String(selectedCount))}
        </Button>
        {selectedNames.length ? <p className="mt-2 truncate text-center text-xs text-muted-foreground">{selectedNames.join(' · ')}</p> : null}
      </div>

      {pending ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-background/85 p-4 backdrop-blur-sm" role="status" aria-live="polite">
          <div className="w-full max-w-md rounded-xl border bg-card p-8 text-center shadow-xl">
            ...
            <div className="mt-5 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full w-2/3 animate-pulse rounded-full bg-primary" /></div>
          </div>
        </div>
      ) : null}
      ...
      {state.code === 'insufficient_credits' && !creditDialogDismissed && !pending ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4">
          <div role="dialog" aria-modal="true" aria-labelledby="credits-dialog-title" className="w-full max-w-md rounded-xl border bg-background p-6 shadow-xl">
            <h2 id="credits-dialog-title" className="text-xl font-semibold">{copy.notEnoughCredits}</h2>
            <p className="mt-2 text-sm text-muted-foreground">{state.message}</p>
            <div className="mt-4 grid grid-cols-2 gap-3 rounded-lg bg-muted/50 p-3 text-center text-sm">
              <div><span className="block text-muted-foreground">{copy.requiredCredits}</span><strong>{state.requiredCredits ?? selectedCount}</strong></div>
              <div><span className="block text-muted-foreground">{copy.availableCredits}</span><strong>{state.availableCredits ?? '—'}</strong></div>
            </div>
            <div className="mt-6 flex justify-end gap-3">
              <Button type="button" variant="outline" onClick={() => setCreditDialogDismissed(true)}>{copy.cancel}</Button>
              <Button asChild><Link href="/credits">{copy.buyCredits}</Link></Button>
            </div>
          </div>
        </div>
      ) : null}
...
function EditorButton({ label, onClick, children }: { label: string; onClick: () => void; children: ReactNode }) {
  return <Button type="button" variant="ghost" size="icon" className="h-8 w-8" aria-label={label} title={label} onClick={onClick}>{children}</Button>;
}
